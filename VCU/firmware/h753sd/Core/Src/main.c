...
const char* dayNames[] = {
    "Unknown", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu", "Minggu"
};
...
int igbtNumeric[] = {
  28480, 28179, 27851, 27497, 27114, 26702, 26261, 25792, 25296, 24775,
  24232, 23671, 23097, 22515, 21933, 21357, 20793, 20250, 19733, 19247,
  18797, 18387, 18017, 17688, 17400, 17151, 16938, 16757, 16609, 16487,
  16387, 16308
};
...
int motorNumeric[] = {
  16857, 16789, 16692, 16569, 16421, 16251, 16061, 15852, 15628, 15391,
  15144, 14890, 14633, 14373, 14111, 13846, 13578, 13308, 13036, 12762,
  12485, 12207, 11927, 11646, 11364, 11080, 10795, 10510, 10225, 9939,
  9654, 9369, 9085, 8802, 8520, 8240, 7962,7687,7414
};
...
//CAN REQUEST
uint8_t canIactual[3] = {0x3d, 0x5f, 13};
uint8_t canLogicErr[3] = {0x3d, 0x8f, 1};
uint8_t canSpeedActual[3] = {0x3d, 0x30, 1};
uint8_t canImaxpk[3] = {0x3d, 0xc4, 1};
uint8_t canTmotor[3] = {0x3d, 0x49, 100};
...
uint8_t canBMS7[2] = {0x12, 0};
uint8_t canBMS8[2] = {0x13, 0};
uint8_t canImu1[2] = {0x02, 131};
uint8_t canImu2[2] = {0x03, 11};
uint8_t canImu3[2] = {0x04, 14};
uint8_t canImu4[2] = {0x05, 21};
uint8_t canRSM[8];
...
volatile uint8_t canVoltmvA[5];
volatile uint8_t canVoltmvB[5];
volatile uint8_t canImc[4];
volatile uint8_t canLogicmc[6];
volatile uint8_t canNmc[4];
volatile uint8_t canIpk[4];
volatile uint8_t Tmotor[4];
volatile uint8_t Tigbt[4];
volatile uint8_t Vdc[4];
volatile uint8_t Vfase[4];
volatile uint8_t Mpower[4];
volatile uint8_t Accel_data[7];
volatile uint8_t Gyro_data[7];
volatile uint8_t Mag_data[7];
volatile uint8_t Angle_data[7];
...
void plotData(void);
int32_t mapClamp(int32_t x, int32_t in_min, int32_t in_max, int32_t out_min, int32_t out_max);
float getIGBTTemperature(int num);
/* USER CODE END PFP */
...
float getIGBTTemperature(int num) {
  for (int i = 0; i < (sizeof(igbtNumeric) / sizeof(igbtNumeric[0])) - 1; i++) {
    if (num >= igbtNumeric[i + 1] && num <= igbtNumeric[i]) {
      float temp = igbtTemperature[i + 1] + (float)(num - igbtNumeric[i + 1]) * (igbtTemperature[i] - igbtTemperature[i + 1]) / (igbtNumeric[i] - igbtNumeric[i + 1]);
      return temp;
    }
  }
  return -999;
}
float getMotorTemperature(int num) {
  for (int i = 0; i < (sizeof(motorNumeric) / sizeof(motorNumeric[0])) - 1; i++) {
    if (num >= motorNumeric[i + 1] && num <= motorNumeric[i]) {
      float temp = motorTemperature[i + 1] + (float)(num - motorNumeric[i + 1]) * (motorTemperature[i] - motorTemperature[i + 1]) / (motorNumeric[i] - motorNumeric[i + 1]);
      return temp;
    }
  }
  return -999;
...
int32_t mapClamp(int32_t x, int32_t in_min, int32_t in_max,
                 int32_t out_min, int32_t out_max)
{
    ...
    return (int32_t)(num / in_range) + out_min;
}
...
uint8_t SD_Log_Init(void)
{
    ...
    snprintf(logFileName, sizeof(logFileName),
             "%s_%02d_%02d-%02d.csv",
             dayNames[currentDate.WeekDay <= 7 ? currentDate.WeekDay : 0],
             currentDate.Date,
...
//	SCB_InvalidateDCache_by_Addr((uint32_t*)adc_data, 32);
	...
	bse = adc_data[1];
	susr = adc_data[2];
	susr = mapClamp(susr, 980, 50000, 0, 100);
	apps1 = adc_data[3];
	apps2 = adc_data[4];
	stel = adc_data[5];
	susl = adc_data[6];
	susl = mapClamp(susl, 980, 50000, 0, 100);
//	adc_data[1] = 121;
}
...
void plotData(){
	...
	arusLV = (canStatusRSM[4] << 8) | canStatusRSM[3];
	teganganLV = (canStatusRSM[6] << 8) | canStatusRSM[5];
	teganganLV = teganganLV;
	IActual = (canImc[2] << 8) | canImc[1];
	IActual = (IActual * 400 )/1070;
	RpmBamo = (canNmc[2] << 8) | canNmc[1];
	RpmBamo = mapClamp(RpmBamo, 0, 32767, 0, 6500);
	statusBamo = (canLogicmc[2] << 8) | canLogicmc[1];
	suhuMotor = (Tmotor[2] << 8) | Tmotor[1];
	suhuMotor = getMotorTemperature(suhuMotor);
	suhuIGBT = (Tigbt[2] << 8) | Tigbt[1];
	...
	teganganHV = teganganHVA + teganganHVB;
	arusBms = (canCurbms[1] << 8) | canCurbms[2];
	Imaxpk = (canIpk[2] << 8) | canIpk[1];
	Imaxpk = mapClamp(Imaxpk, 0, 16384, 0, 424);
	roll  = (Angle_data[2] << 8) | Angle_data[1];
...
void loopUtama(void){
	...
	canVci[4] = canNmc[1];
	canVci[5] = canNmc[2];
	canVci[6] = canLogicmc[1];
	canVci[7] = canLogicmc[2];
	canVci2[0]=susl;
	canVci2[1]=susr;
	canVci2[2]=suhuMotor;
	canVci2[3]=suhuIGBT;
	...
	if(HAL_GetTick() - vciInterval2 >= 33){
//		CANMC_Send(0x201, canVphase, 3);
		CANALL_Send(7, canVci2, 8);
		vciInterval2 = HAL_GetTick();
	}
